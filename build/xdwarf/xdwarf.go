...
var (
	...
	errNoStrSection   = errors.New("string form needs a section the caller did not supply")
)
...
// Sections addresses the DWARF section payloads a reader needs. They are read
// through rather than loaded: a line program is consumed once, forward, and the
// opcode stream is the bulk of it, so holding the whole section resident buys
// nothing. Callers fill in what they have; an operation that needs a section
// absent here fails rather than guessing.
//
// Relocation is the caller's business, and matters for an object whose debug
// sections carry .rel entries -- TinyGo emits them. Wrap the reader in
// [xelf.RelocReaderAt] to have the fixups applied as the bytes are read.
//
// Obtain the readers with xelf, whose FileSection.Open returns an
// [io.SectionReader]:
//
//	sec, _ := f.SectionByName(".debug_line")
//	s.Line = sec.Open()
//
// An in-memory caller wraps its bytes with [bytes.NewReader]. The size of
// .debug_line is passed separately, to [DecodeLineUnit].
//
// [xelf.RelocReaderAt]: github.com/soypat/tinyboot/build/xelf.RelocReaderAt
type Sections struct {
	Line      io.ReaderAt // .debug_line
	LineStr   io.ReaderAt // .debug_line_str, DWARF 5 only.
	Str       io.ReaderAt // .debug_str
